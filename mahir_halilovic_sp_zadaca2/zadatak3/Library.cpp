...
void Library::CreateUser() {
  ...
  auto it = find_if(users_.begin(), users_.end(), [&id](const User &obj) {return obj.getID() == id;});
  if(it == users_.end()) {
...
void Library::BookAdd() {
  ...
  auto it = find_if(books_.begin(), books_.end(), [&t](const Book &obj){ return obj.getTitle() == t; });
...
void Library::BookBorrow() {
  ...
  if(it == users_.end()) {
    ...
    auto it2 = find_if(books_.begin(), books_.end(), [&title](const Book &obj){ return obj.getTitle() == title; });
    if(it2 != books_.end()) {
      it2->borrow();
      it->borrow(title);
    }  else
...
void Library::BookSearchByTitle() {
  ...
  auto it = find_if(books_.begin(), books_.end(), [&title](const Book &obj){ return obj.getTitle() == title; });
